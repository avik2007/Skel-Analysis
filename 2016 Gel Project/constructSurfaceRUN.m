...
for num = 1: numTotal
    ...
    pixelprojectZ = paramLengthDetected(num) * unitVectors_Z(num) * xy2zDistConversion + centroid_nna(3);
    
    distprojectX = pixelprojectX * xy_scale_factor;
    distprojectY = pixelprojectY * xy_scale_factor;
    distprojectZ = pixelprojectZ * z_scale_factor;
...
figure()
plot3(surfX(2:numTotal+1,1),surfY(2:numTotal+1,1), surfZ(2:numTotal+1,1),'.','Markersize',7); 
hold on
plot3(surfX(1,1), surfY(1,1), surfZ(1,1), 'xr');
axis equal
axis vis3d
